...
model_save_path = 'output-s/5q_Q2AQtrain_bi-encoder-mnrl-{}-margin_{:.1f}-{}'.format(model_name.replace("/", "-"), ce_score_margin, datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))
...
collection_filepath = os.path.join(data_folder, 'Only_5q.tsv')
...
class MSMARCODataset(Dataset):
    def __init__(self, queries, corpus,N_queries_added):
        self.queries = queries
        self.queries_ids = list(queries.keys())
        self.corpus = corpus
        self.N = N_queries_added
        self.N_msmarco = 8841823 # length of each query set generated from ms-marco corpus (length of original corpus before extension)

        for qid in self.queries:
            self.queries[qid]['pos'] = self.extend_pos_ids(list(self.queries[qid]['pos'])) # TODO check the function
            self.queries[qid]['neg'] = list(self.queries[qid]['neg'])
            random.shuffle(self.queries[qid]['neg'])
    def put_in_range(self, item):
        return item % len(self.queries) ## TODO check if this should be corpus or query
    def extend_pos_ids(self, pos_id_list):

        ext = []
        for pid in pos_id_list:
            ext.extend([self.N_msmarco*i + pid for i in range(10)])
        return ext
    def __getitem__(self, item):
        item = self.put_in_range(item)
    ...
    def __len__(self):
        return self.N*len(self.queries)

train_dataset = MSMARCODataset(train_queries, corpus=corpus, N_queries_added=10)
